...
for subj in subjs:
    ...
    for i in range(len(subj_df)):#
        ...
        if data_start_file_name == data_end_file_name: # if this meal is in one hour's file:
            try:
                ...
                print('FILE NOT EXSIT: ', os.path.join(necklace_path, data_start_file_name))
                continue
        else:
    ...
    subj_df.to_csv('/Users/shibozhang/Documents/Beyourself/beyourself-label/BeYourself/CLEAN/'+subj+'/labeling_'+subj+'_for_analysis_necklace_reliability.csv',index = None)
